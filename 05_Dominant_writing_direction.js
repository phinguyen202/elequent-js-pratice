var SCRIPTS = [
  ...
  // loop for each character of text
  // find the script of character, count and add to array
  // find the script has highlest count
  // show dominant  writting direction
  let str = "俄abc";
  
  function compute(text) {
    let count = [];
    for (let t of text) {
      script = SCRIPTS.find(c => c.ranges.some(([from, to]) => (from <= t.codePointAt() && to > t.codePointAt())));
      if (script) {
        if (count.length === 0 || count.findIndex(e => e.name === script.name) === -1) {
          count.push({name: script.name, count: 1, direction: script.direction});
        } else {
          count.find(e => e.name === script.name).count++;
        }
      }
    }
    return count;
  ...
  console.log(compute("Hey, مساء الخير").reduce((a,b) => a.direction >= b.direction ? a : b).direction);